...
Page {
    ...
    // --- data helpers ------------------------------------------------------
    // Only readings with a trustworthy timestamp can go on a time axis. Ones
    // taken while the monitor's clock was unset all share the same factory
    // stamp and would otherwise pile up on a single column.
    ...
    function ofPerson() {
        var rev = omron.personRevision
        var ms = omron.measurements, out = []
        for (var i = 0; i < ms.length; ++i) {
            var p = ms[i].person === 2 ? 2 : 1
            if (p === omron.chartPerson) out.push(ms[i])
        }
        return out
    }
    function dated() {
        var ms = page.ofPerson(), out = []
        for (var i = 0; i < ms.length; ++i)
            if (ms[i].timeValid === undefined || ms[i].timeValid) out.push(ms[i])
        return out
    }
    function undatedCount() { return page.ofPerson().length - dated().length }

    // Counted here rather than through a C++ method: a binding only re-runs
    // when a *property* it read changes, and omron.measurements is one —
    // an invokable's result would go stale the moment a reading is deleted.
    function countOf(p) {
        var rev = omron.personRevision
        var ms = omron.measurements, n = 0
        for (var i = 0; i < ms.length; ++i)
    ...
    function dataRange() {
        var ms = page.dated(), minE = -1, maxE = 0
        for (var i = 0; i < ms.length; ++i) {
            var e = ms[i].epoch
            if (e <= 0) continue
    ...
    // yield undefined, making every tap count as a drag. Small, so the drag is
    // claimed before the page stack decides the swipe means "back to the list".
    ...
    SilicaFlickable {
        ...
        PullDownMenu {
            MenuItem {
                ...
                enabled: omron.count > 0
                onClicked: chartArea.grabToImage(function(res) {
                    var p = omron.reserveImagePath()
                    if (res.saveToFile(p)) omron.notify(qsTr("Image saved: ") + p)
                    else omron.notify(qsTr("Image export failed"))
                })
            }
            MenuItem {
        ...
        Column {
            ...
            Label {
                ...
                visible: page.undatedCount() > 0
                font.pixelSize: Theme.fontSizeExtraSmall
                color: "#FB8C00"
                text: qsTr("%1 reading(s) not shown: taken while the monitor's clock "
                         + "was unset, so they have no date. Device → Set the "
                         + "monitor's clock fixes this for future readings.")
                        .arg(page.undatedCount())
            }
                ...
                Row {
                    ...
                    Repeater {
                        ...
                        delegate: Rectangle {
                            ...
                            radius: height / 2
                            property bool active: omron.chartPerson === modelData
